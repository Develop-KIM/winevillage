	...
	<section id="contents" class="mb_margin_0" style="margin-top: 188px;">
			...
			<div class="content order cart_lists_page">
					...
					<ul>
						...
						<li class="65402">
							<div class="box ip_img">
								<div class="checkbox type2">
									<input type="checkbox" id="cart_seq_65402" name="cart_seq[]"
										class="ip_check" value="65402" data-qty="1" data-price="14900"
										data-supply-price="14900" data-promotion-limit="N"> <label
										for="cart_seq_65402"> <picture
											style="background:#E0D8EA"> <!--[if IE 9]><video style="display: none;"><![endif]-->
										<source
											srcset="/uploads/product/aa7a7a81345a0f66e00a0962f5ab3904.png"
										...
										<!-- mb이미지 --> <!--[if IE 9]></video><![endif]--> <img
											src="/uploads/product/aa7a7a81345a0f66e00a0962f5ab3904.png"
											alt=""><!-- pc이미지 --> </picture>
									</label>
								</div>
							</div>
							<div class="box con">
								<div class="more_info">
									<p class="prd_name">
										<a href="/shop/product/product_view?product_cd=03U001"
											target="_blank"> 디아블로 데블스 카나발 카베르네<br>
										</a>
									</p>

									<div class="cate_label">
										<span class="label" style="background: #E0D8EA">레드</span> <span
											class="label" style="background: #E0D8EA">칠레</span> <span
											class="label" style="background: #E0D8EA">카베르네 소비뇽</span>
									</div>
								</div>
							</div>
							<div class="box opt">
							...
							<div class="box price_amount">
								<div class="box amount">
									<div class="quantity" data-cart-seq="65402"
										data-product-cd="03U001" data-base-price="14900"
										data-opt-gb="C">
										<button type="button" class="minus"
											onclick="box_qty(this, -1);">감소</button>
										<input type="text" class="qty" title="수량" value="1">
										<button type="button" class="plus"
											onclick="box_qty(this, +1);">증가</button>
									</div>
								</div>
				...
				<div class="btn_area col2">
					<button type="button" class="btn_txt"
						onclick="location.href='/shop/product/product_lists?sh_category1_cd=10000'">
						<span>계속 쇼핑하기</span>
					</button>
					<button type="button" class="btn_txt"
						onclick="location.href='/order_write.do'">
						<span>선택상품 주문</span>
					</button>
		...
		<form action="https://www.winenara.com/shop/cart/cart_lists"
			...
			<input type="hidden" name="witplus_csrf_token"
				value="1f07d7574108f334993e9bc5182fc2ea"> <input
		...
		<script>
			...
			$("#btn_del")
					.on(
							...
							function() {
								...
								var cart_val = [];
								var cart_opt_val = [];
								if ($("input[name='cart_seq[]']:checked").length > 0
										|| $("input[name='cart_opt_seq[]']:checked").length > 0) {
									$("input[name='cart_seq[]']:checked").each(
											function(idx) {
												cart_val.push($(this).val());
											});

									$("input[name='cart_opt_seq[]']:checked")
											.each(
													function(idx) {
														cart_opt_val.push($(
																this).val());
									...
									if ($.trim(res.status) == "ok") {
										...
										if ($.trim(res.status) == "err2") {
											if (res.data.length > 0) {
												var stock = parseInt(res.data[0].stock)
														- parseInt(res.data[0].limit_cnt);

												alert("재고가 부족합니다. 현재 남아있는 재고의 수는 "
														+ stock + "개 입니다.");
												location.reload();
											}
										} else if ($.trim(res.status) == "err3") {
											alert('해당 옵션의 최소 수량은 ' + res.data
													+ '개 이상입니다. ');
											return;
										} else {
											alert(res.msg);
			...
			function orderSet(state) {
				...
				$("input:checkbox[name='cart_seq[]']:checked").each(
						function(idx) {
						...
						if ($.trim(res.status) == "ok") {
							...
							if (res.data.length > 0) {
								for (var i = 0; i < res.data.length; i++) {
									if (msg) {
										msg += ", \n" + res.data[i].product_nm
												+ res.data[i].opt_nm2
									} else {
										msg = res.data[i].product_nm
												+ res.data[i].opt_nm2
									}
								}
								msg += "의 재고가 부족 합니다.";
